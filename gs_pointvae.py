...
class Network(nn.Module):
    def __init__(self):
        ...
        self.encoder = GS_encoder(4,256,resol**2,[4],16384)
        self.decoder = GS_decoder(4,256,16384,[4],14*resol**2)
...
for epoch in tqdm(range(num_epochs)):
       ...
       for bch in range(0, len(folder_path_each), bch_size):
           ...
           loss_render = 0.0
           optimizer.zero_grad()
           mu, log_var, z, UV_gs_recover = gs_autoencoder(UV_gs_batch.reshape([bch_size, -1, 14]))
           ...
           if loss_usage == "L1":
              loss += torch.norm(UV_gs_recover.reshape([bch_size, -1, 14]) - UV_gs_batch.reshape([bch_size, -1, 14]), p=2)/len(UV_gs_batch) + 0.0001*KL_loss + 10*loss_render
